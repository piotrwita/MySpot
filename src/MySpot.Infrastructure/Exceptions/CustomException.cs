    ...
    private async Task HandleExceptionAsync(Exception exception, HttpContext context)
    {
        var (statusCode, error) = exception switch
        {
            CustomException => (StatusCodes.Status400BadRequest, new Error(exception
                 //unserscore z paczki humanizer zmienia string na snakecare
                 //nazwe pobieram z getytype czyli nazwy klasy i pozbywam sie slowa Exception
                 //finalnie mam nazwe bledu
